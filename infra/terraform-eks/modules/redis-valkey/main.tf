...
# 2. Security Group (Portão de entrada) - acesso restrito ao SG do cluster EKS
resource "aws_security_group" "valkey_sg" {
  name        = "valkey-sg"
  description = "Permitir acesso ao Valkey (sinal de conclusao da efetivacao do ledger) a partir dos pods do EKS"
  vpc_id      = var.vpc_id
