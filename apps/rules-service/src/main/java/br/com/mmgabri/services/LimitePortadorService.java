...
public class LimitePortadorService {
    ...
    @SneakyThrows
    public LimitePortadorResponse execute(LimitePortadorRequest request) {

        if (request.getSleepLimitePortador() > 0) {
            sleep(Duration.ofMillis(request.getSleepLimitePortador()));
        } else {
            sleep(Duration.ofMillis(customSleepMillis));
        }

        if ("999".equals(request.getCustomReturnLimitePortador())) {
        ...
        LimitePortadorResponse response = LimitePortadorResponse.newBuilder()
                .setHeaderMessageGrpc(header)
                .setAprovado("000".equals(request.getCustomReturnLimitePortador()))
                .setErrorCode(request.getCustomReturnLimitePortador())
                .setErrorDescription(getMessage(request.getCustomReturnLimitePortador()))
