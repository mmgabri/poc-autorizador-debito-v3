...
public class UseCase {
    ...
    public AutorizadorResponse execute(Payload payload) {
        ...
        if (serviceSucceeded(antiFraudFuture) || serviceSucceeded(limitEfetFuture) || serviceSucceeded(ledgerEfetFuture)) {
            logger.warn("Phase 2 partial failure - publishing reversal notification");
            sqsReversalNotificationService.publish(payload, serviceSucceeded(limitEfetFuture), serviceSucceeded(ledgerEfetFuture), (serviceSucceeded(antiFraudFuture)));
        }
    ...
    private <T> CompletableFuture<T> handleCompletion(CompletableFuture<T> future, TransactionExecutionContext txCtx, ServicesEnum service) {
        return future.whenComplete((_result, ex) -> {
            if (ex == null) {
                ...
                logger.error("Service '{}' failed: {}", service.getServiceName(), cause.getMessage());
    ...
    private Optional<Throwable> findFirstError(CompletableFuture<?>... futures) {
        return Arrays.stream(futures)
                ...
                .map(future -> future.handle((result, ex) -> ex).join())
